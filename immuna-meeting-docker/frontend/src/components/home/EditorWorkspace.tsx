...
export default function EditorWorkspace({ onProcess, isLoading, error }: EditorWorkspaceProps) {
  ...
  const [immunizationProfile, setImmunizationProfile] = useState<ImmunizationProfile>("nano_banana_2");
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm leading-relaxed" style={{ color: "var(--muted-foreground)" }}>
            ...
            upgraded API. The prepared editor keeps the whole photo frame instead of cropping
            everything to a square.
          </p>
        </div>
        <div
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[300px_minmax(0,1fr)] gap-6">
            ...
            >
              <div>
                ...
                <p className="text-xs mt-1 leading-relaxed" style={{ color: "var(--muted-foreground)" }}>
                  Optional per-run overrides. Leave fields blank to keep the selected profile defaults.
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                >
                  ...
                  <div>
                    ...
                    <span className="text-xs" style={{ color: "var(--muted-foreground)" }}>
                      Fails closed instead of allowing lower-memory fallbacks.
                    </span>
                  </div>
                </label>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <label className="text-sm" style={{ color: "var(--foreground)" }}>
                  <span className="block font-semibold mb-2">Profile</span>
                  <select
                  ...
                  <span className="block text-xs mt-2" style={{ color: "var(--muted-foreground)" }}>
                    {selectedProfileLabel}: {selectedProfileSummary}
                  </span>
                </label>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <label className="text-sm" style={{ color: "var(--foreground)" }}>
                  <span className="block font-semibold mb-2">Immunization iterations</span>
                  <input value={immunizationIters} onChange={(event) => setImmunizationIters(event.target.value)} className="w-full rounded-2xl px-4 py-3" style={{ backgroundColor: "var(--card)", color: "var(--foreground)", border: "1px solid var(--border)" }} />
                </label>
                <label className="text-sm" style={{ color: "var(--foreground)" }}>
                  <span className="block font-semibold mb-2">EOT samples</span>
                  <input value={eotSamples} onChange={(event) => setEotSamples(event.target.value)} className="w-full rounded-2xl px-4 py-3" style={{ backgroundColor: "var(--card)", color: "var(--foreground)", border: "1px solid var(--border)" }} />
                </label>
                <label className="text-sm" style={{ color: "var(--foreground)" }}>
                  <span className="block font-semibold mb-2">Max prompt variants</span>
                  <input value={maxPromptVariants} onChange={(event) => setMaxPromptVariants(event.target.value)} className="w-full rounded-2xl px-4 py-3" style={{ backgroundColor: "var(--card)", color: "var(--foreground)", border: "1px solid var(--border)" }} />
                </label>
                <label className="text-sm" style={{ color: "var(--foreground)" }}>
                  <span className="block font-semibold mb-2">Denoiser strength</span>
                  <input value={denoiserStrength} onChange={(event) => setDenoiserStrength(event.target.value)} className="w-full rounded-2xl px-4 py-3" style={{ backgroundColor: "var(--card)", color: "var(--foreground)", border: "1px solid var(--border)" }} />
                </label>
                <label className="text-sm" style={{ color: "var(--foreground)" }}>
                  <span className="block font-semibold mb-2">Reference confusion</span>
                  <input value={referenceConfusionStrength} onChange={(event) => setReferenceConfusionStrength(event.target.value)} className="w-full rounded-2xl px-4 py-3" style={{ backgroundColor: "var(--card)", color: "var(--foreground)", border: "1px solid var(--border)" }} />
                </label>
                <label className="text-sm" style={{ color: "var(--foreground)" }}>
                  <span className="block font-semibold mb-2">Identity drift</span>
                  <input value={identityDriftStrength} onChange={(event) => setIdentityDriftStrength(event.target.value)} className="w-full rounded-2xl px-4 py-3" style={{ backgroundColor: "var(--card)", color: "var(--foreground)", border: "1px solid var(--border)" }} />
                </label>
                <label className="text-sm" style={{ color: "var(--foreground)" }}>
                  <span className="block font-semibold mb-2">Semantic boundary</span>
                  <input value={semanticBoundaryStrength} onChange={(event) => setSemanticBoundaryStrength(event.target.value)} className="w-full rounded-2xl px-4 py-3" style={{ backgroundColor: "var(--card)", color: "var(--foreground)", border: "1px solid var(--border)" }} />
                </label>
                <label className="text-sm" style={{ color: "var(--foreground)" }}>
                  <span className="block font-semibold mb-2">Watermark strength</span>
                  <input value={watermarkStrength} onChange={(event) => setWatermarkStrength(event.target.value)} className="w-full rounded-2xl px-4 py-3" style={{ backgroundColor: "var(--card)", color: "var(--foreground)", border: "1px solid var(--border)" }} />
                </label>
                <label className="text-sm" style={{ color: "var(--foreground)" }}>
                  <span className="block font-semibold mb-2">Tripwire global</span>
                  <input value={tripwireGlobalStrength} onChange={(event) => setTripwireGlobalStrength(event.target.value)} className="w-full rounded-2xl px-4 py-3" style={{ backgroundColor: "var(--card)", color: "var(--foreground)", border: "1px solid var(--border)" }} />
                </label>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-5">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <Button onClick={handleClearMask} variant="secondary" disabled={!selectedImage || isLoading}>
              Clear mask
            </Button>
          </div>

          {combinedError ? (
            <p className="text-sm whitespace-pre-line" style={{ color: "var(--destructive)" }}>
              {combinedError}
            </p>
          ) : (
            <p className="text-sm" style={{ color: "var(--muted-foreground)" }}>
              The protected region is defined by the white mask. Keep the prompt filled in if you want to test the edit step too.
            </p>
          )}
        </div>
      </div>
