...
def _candidate_local_model_dirs() -> list[Path]:
    ...
    candidates.extend([
        ...
        Path("/workspace/artifacts/sdxl_local_inpaint_model"),
        Path("/workspace/artifacts/local_inpaint_model"),
        Path("/workspace/photoguard/artifacts/sdxl_local_inpaint_model"),
        Path("/workspace/photoguard/artifacts/local_inpaint_model"),
        REPO_ROOT / "artifacts" / "sdxl_local_inpaint_model",
        REPO_ROOT / "artifacts" / "local_inpaint_model",
        UPGRADED_ROOT / "artifacts" / "sdxl_local_inpaint_model",
...
from vision_surrogates import (  # noqa: E402
    get_face_id_surrogate_status,
    get_vision_surrogate_status,
...
PIPELINE_UNET_CHECKPOINTING_ENABLED = _env_flag("PHOTOGUARD_ENABLE_UNET_CHECKPOINTING", True)
PIPELINE_VAE_CHECKPOINTING_ENABLED = _env_flag("PHOTOGUARD_ENABLE_VAE_CHECKPOINTING", True)
...
METRIC_LABELS = {
    ...
    "clip": "CLIP vision confusion",
    "clip_purify": "CLIP purification confusion",
    "denoiser": "Proxy denoiser drift",
    "dino": "DINO vision confusion",
    "dino_purify": "DINO purification confusion",
    "face_id": "Face-ID confusion",
    "face_id_purify": "Face-ID purification confusion",
    ...
    "purify": "Teacher purification robustness",
...
class HealthResponse(BaseModel):
    ...
    runtimeSettings: dict[str, object] = Field(default_factory=dict)
    surrogateStatus: dict[str, dict[str, object]] = Field(default_factory=dict)
...
def _build_surrogate_runtime_status(*, probe: bool = False) -> dict[str, dict[str, object]]:
    device = torch.device(PIPELINE_DEVICE if PIPELINE_DEVICE == "cuda" and torch.cuda.is_available() else "cpu")
    if probe:
        maybe_load_vision_surrogate("clip", device)
        maybe_load_vision_surrogate("dino", device)
        maybe_load_face_id_surrogate(device)
    return {
        "clip": get_vision_surrogate_status("clip", device),
        "dino": get_vision_surrogate_status("dino", device),
        "faceId": get_face_id_surrogate_status(device),
    }
...
def _configure_pipeline_memory(pipeline: object) -> dict[str, object]:
    ...
    if PIPELINE_DEVICE == "cuda":
        ...
        try:
            ...
            pass
        try:
            if PIPELINE_UNET_CHECKPOINTING_ENABLED and hasattr(pipeline.unet, "enable_gradient_checkpointing"):
                pipeline.unet.enable_gradient_checkpointing()
                features["unet_gradient_checkpointing"] = True
        except Exception as exc:
            features["unet_gradient_checkpointing_error"] = type(exc).__name__
        try:
            if PIPELINE_VAE_CHECKPOINTING_ENABLED and hasattr(pipeline.vae, "enable_gradient_checkpointing"):
...
def _immunization_config_for_profile(profile_name: str, target_size: tuple[int, int]) -> ImmunizationConfig:
    ...
    if profile_name == "nano_banana_2":
        return replace(
            ...
            denoiser_strength=0.08,
            denoiser_steps=1,
            denoiser_early_timestep_bias=1.1,
            eot_samples=2,
            resize_jitter=0.03,
            ...
            clip_vision_confusion_strength=0.08,
            dino_vision_confusion_strength=0.1,
            surrogate_face_id_strength=0.08,
            updown_scale_jitter=0.18,
            sharpen_proxy_strength=0.16,
            teacher_purification_strength=0.12,
            teacher_purification_view_count=2,
            max_prompt_variants=10,
            low_memory_loss_scale=0.4,
        )
    if profile_name == "surrogate_hybrid":
        return replace(
            config,
            eps=0.072,
            step_size=0.0054,
            ...
            denoiser_steps=1,
            denoiser_early_timestep_bias=1.3,
            eot_samples=2,
            resize_jitter=0.04,
            noise_strength=0.004,
            blur_kernel_size=3,
            mask_augmentation_strength=0.1,
            mask_augmentation_count=1,
            semantic_boundary_strength=0.1,
            semantic_ring_width=12,
            compression_jitter_strength=0.05,
            subpixel_jitter=0.28,
            frequency_noise_strength=0.0035,
            frequency_band_low=0.08,
            frequency_band_high=0.26,
            watermark_strength=0.09,
            ...
            reference_confusion_strength=0.15,
            identity_drift_strength=0.12,
            portrait_face_identity_strength=0.09,
            surrogate_face_id_strength=0.1,
            clip_vision_confusion_strength=0.08,
            dino_vision_confusion_strength=0.1,
            context_blend_strength=0.06,
            reference_region_count=3,
            multiscale_descriptor_strength=0.07,
            multiscale_descriptor_scales=(1.0, 0.5),
            background_anchor_confusion_strength=0.05,
            background_anchor_count=3,
            tripwire_strength=0.08,
            tripwire_ring_strength=0.05,
            tripwire_anchor_strength=0.03,
            tripwire_anchor_count=3,
            tripwire_global_strength=0.06,
            tripwire_global_count=2,
            updown_scale_jitter=0.18,
            sharpen_proxy_strength=0.16,
            teacher_purification_strength=0.12,
            teacher_purification_view_count=2,
            max_prompt_variants=8,
            low_memory_loss_scale=0.4,
        )
    if profile_name == "surrogate_deevid":
        return replace(
            config,
            eps=0.082,
            step_size=0.0062,
            ...
            region_priority_strength=0.06,
            priority_face_strength=0.4,
            priority_skin_strength=0.24,
            priority_text_strength=0.18,
            priority_logo_strength=0.18,
            reference_confusion_strength=0.08,
            identity_drift_strength=0.06,
            portrait_face_identity_strength=0.06,
            surrogate_face_id_strength=0.0,
            clip_vision_confusion_strength=0.03,
            dino_vision_confusion_strength=0.04,
            context_blend_strength=0.0,
            reference_region_count=2,
            multiscale_descriptor_strength=0.0,
            background_anchor_confusion_strength=0.0,
            background_anchor_count=0,
            tripwire_strength=0.05,
            tripwire_ring_strength=0.03,
            tripwire_anchor_strength=0.0,
            tripwire_anchor_count=0,
            tripwire_global_strength=0.06,
            tripwire_global_count=2,
            updown_scale_jitter=0.06,
            sharpen_proxy_strength=0.04,
            teacher_purification_strength=0.04,
            teacher_purification_view_count=1,
            max_prompt_variants=6,
            low_memory_loss_scale=0.45,
        )
...
def _low_memory_immunization_config(config: ImmunizationConfig) -> ImmunizationConfig:
    ...
    return replace(
        ...
        surrogate_face_id_strength=min(config.surrogate_face_id_strength, 0.05) * scale,
        clip_vision_confusion_strength=min(config.clip_vision_confusion_strength, 0.05) * scale,
        dino_vision_confusion_strength=min(config.dino_vision_confusion_strength, 0.06) * scale,
        ...
        teacher_purification_strength=min(config.teacher_purification_strength, 0.08) * scale,
...
def health(probe_surrogates: bool = False, probe_pipeline: bool = False) -> HealthResponse:
    pipeline_error = _pipeline_error
    if probe_pipeline and _pipeline is None:
        try:
            _get_pipeline()
        except Exception as exc:
            pipeline_error = str(exc)
    return HealthResponse(
        status="ok" if pipeline_error is None else "offline",
        device=PIPELINE_DEVICE,
        modelSource=INPAINT_MODEL_SOURCE,
        pipelineReady=_pipeline is not None,
        pipelineError=pipeline_error,
        runtimeSettings={
            "pipelineFamily": _pipeline_runtime_features.get("family"),
            "xformersEnabled": bool(_pipeline_runtime_features.get("xformers_enabled")),
            "compileEnabled": bool(_pipeline_runtime_features.get("compile_enabled")),
            "unetCheckpointingEnabled": PIPELINE_UNET_CHECKPOINTING_ENABLED,
            "vaeCheckpointingEnabled": PIPELINE_VAE_CHECKPOINTING_ENABLED,
            "requireFullStrength": REQUIRE_FULL_STRENGTH_IMMUNIZATION,
            "localFilesOnly": INPAINT_LOCAL_FILES_ONLY,
            "torchCudaAvailable": bool(torch.cuda.is_available()),
            "pipelineInitialized": _pipeline is not None or pipeline_error is not None,
        },
        surrogateStatus=_build_surrogate_runtime_status(probe=probe_surrogates),
    )
