...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Optional real face-ID confusion strength for the surrogate branch.",
    )
    parser.add_argument(
        "--clip-confusion",
        type=float,
        default=0.0,
        help="CLIP vision-encoder confusion strength.",
    )
    parser.add_argument(
        "--dino-confusion",
        type=float,
        default=0.0,
        help="DINOv2 vision-encoder confusion strength.",
    )
    parser.add_argument(
        "--teacher-purification",
        type=float,
        default=0.0,
        help="Teacher-time cleanup/purification robustness strength.",
    )
    parser.add_argument(
        "--teacher-purification-views",
        type=int,
        default=0,
        help="Number of teacher-time purification proxy views.",
    )
    parser.add_argument(
        "--mask-variant-count",
        type=int,
            ...
            for prompt_index, prompt in enumerate(prompts, start=1):
                ...
                try:
                    ...
                    immunization_config = replace(
                        ...
                        surrogate_face_id_strength=float(
                            row.get("surrogate_face_id", args.surrogate_face_id)
                        ),
                        clip_vision_confusion_strength=float(
                            row.get("clip_confusion", args.clip_confusion)
                        ),
                        dino_vision_confusion_strength=float(
                            row.get("dino_confusion", args.dino_confusion)
                        ),
                        teacher_purification_strength=float(
                            row.get("teacher_purification", args.teacher_purification)
                        ),
                        teacher_purification_view_count=int(
                            row.get(
                                "teacher_purification_views",
                                args.teacher_purification_views,
                            )
                        ),
                    )
