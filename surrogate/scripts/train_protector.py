...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--clip-feature-weight", type=float, default=None, help="Weight for CLIP vision confusion on the protected region. Defaults to a manifest-aware surrogate value.")
    parser.add_argument("--clip-purification-weight", type=float, default=None, help="Weight for CLIP confusion after purification proxies. Defaults to a manifest-aware surrogate value.")
    parser.add_argument("--dino-feature-weight", type=float, default=None, help="Weight for DINOv2 vision confusion on the protected region. Defaults to a manifest-aware surrogate value.")
    parser.add_argument("--dino-purification-weight", type=float, default=None, help="Weight for DINOv2 confusion after purification proxies. Defaults to a manifest-aware surrogate value.")
...
def _teacher_uses_surrogate_losses(rows: list[dict[str, Any]]) -> dict[str, bool]:
    summary = {
        "clip": False,
        "dino": False,
        "face_id": False,
    ...
    for row in rows:
        config = row.get("teacherConfig") or {}
        try:
            summary["clip"] = summary["clip"] or float(config.get("clip_vision_confusion_strength", 0.0)) > 0
            summary["dino"] = summary["dino"] or float(config.get("dino_vision_confusion_strength", 0.0)) > 0
            summary["face_id"] = summary["face_id"] or float(config.get("surrogate_face_id_strength", 0.0)) > 0
            summary["purification"] = summary["purification"] or float(config.get("teacher_purification_strength", 0.0)) > 0
        except (TypeError, ValueError):
            continue
    return summary


def _apply_surrogate_student_defaults(args: argparse.Namespace, rows: list[dict[str, Any]]) -> None:
    teacher_usage = _teacher_uses_surrogate_losses(rows)
    defaults_applied: list[str] = []

    if args.clip_feature_weight is None:
        args.clip_feature_weight = 0.05 if teacher_usage["clip"] else 0.0
        if teacher_usage["clip"]:
            defaults_applied.append("clip_feature_weight=0.05")
    if args.clip_purification_weight is None:
        args.clip_purification_weight = 0.03 if teacher_usage["clip"] and teacher_usage["purification"] else 0.0
        if teacher_usage["clip"] and teacher_usage["purification"]:
            defaults_applied.append("clip_purification_weight=0.03")
    if args.dino_feature_weight is None:
        args.dino_feature_weight = 0.08 if teacher_usage["dino"] else 0.0
        if teacher_usage["dino"]:
            defaults_applied.append("dino_feature_weight=0.08")
    if args.dino_purification_weight is None:
        args.dino_purification_weight = 0.04 if teacher_usage["dino"] and teacher_usage["purification"] else 0.0
        if teacher_usage["dino"] and teacher_usage["purification"]:
            defaults_applied.append("dino_purification_weight=0.04")

    if defaults_applied:
        print(
            "info: applied manifest-aware surrogate student defaults: " + ", ".join(defaults_applied),
            file=sys.stderr,
        )


def _warn_on_surrogate_mismatch(args: argparse.Namespace, rows: list[dict[str, Any]]) -> None:
    teacher_usage = _teacher_uses_surrogate_losses(rows)
    if (
        (teacher_usage["clip"] or teacher_usage["dino"])
        and args.clip_feature_weight == 0.0
        and args.clip_purification_weight == 0.0
        and args.dino_feature_weight == 0.0
        and args.dino_purification_weight == 0.0
    ):
        print(
            "warning: teacher rows use CLIP/DINO surrogate losses, but student CLIP/DINO weights are all zero; "
            "this run will distill only the image-space teacher and real FaceNet path.",
            file=sys.stderr,
        )
    if teacher_usage["face_id"] and args.face_id_backend == "none":
        print(
            "warning: teacher rows use surrogate FaceNet loss, but --face-id-backend=none disables the real face-ID branch in student training.",
            file=sys.stderr,
        )
...
def _maybe_build_face_id_model(backend: str, device: torch.device) -> nn.Module | None:
    if backend == "none":
        return None
    if backend in {"auto", "facenet"}:
        return maybe_load_face_id_surrogate(device)
    return None


def _maybe_build_vision_surrogate(
    kind: str,
    *,
    feature_weight: float,
    purification_weight: float,
    device: torch.device,
) -> VisionSurrogate | None:
    if max(float(feature_weight), float(purification_weight)) <= 0.0:
        return None
    return maybe_load_vision_surrogate(kind, device)
...
def _compute_vision_surrogate_similarity(
...
def _evaluate(
    ...
    *,
    ...
    clip_surrogate: VisionSurrogate | None,
    dino_surrogate: VisionSurrogate | None,
    ...
    metrics = {
        ...
        "clip_confusion": 0.0,
        "clip_purification": 0.0,
        "dino_confusion": 0.0,
        "dino_purification": 0.0,
        "purification": 0.0,
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            clip_confusion_loss = _compute_vision_surrogate_similarity(
                clip_surrogate,
                source,
                pred_protected,
                protected_mask,
            )
            dino_confusion_loss = _compute_vision_surrogate_similarity(
                dino_surrogate,
                source,
                pred_protected,
                protected_mask,
            ...
            clip_purification_loss = _compute_vision_surrogate_similarity(
                clip_surrogate,
                source,
                purified_pred,
                protected_mask,
            )
            dino_purification_loss = _compute_vision_surrogate_similarity(
                dino_surrogate,
                source,
                purified_pred,
                protected_mask,
            ...
            total_loss = (
                ...
                + args.clip_feature_weight * clip_confusion_loss
                + args.clip_purification_weight * clip_purification_loss
                + args.dino_feature_weight * dino_confusion_loss
                + args.dino_purification_weight * dino_purification_loss
            ...
            metrics["clip_confusion"] += float(clip_confusion_loss.item())
            metrics["clip_purification"] += float(clip_purification_loss.item())
            metrics["dino_confusion"] += float(dino_confusion_loss.item())
            metrics["dino_purification"] += float(dino_purification_loss.item())
...
def main() -> int:
    ...
    _apply_surrogate_student_defaults(args, rows)
    _warn_on_surrogate_mismatch(args, rows)
    ...
    clip_surrogate = _maybe_build_vision_surrogate(
        "clip",
        feature_weight=args.clip_feature_weight,
        purification_weight=args.clip_purification_weight,
        device=device,
    )
    dino_surrogate = _maybe_build_vision_surrogate(
        "dino",
        feature_weight=args.dino_feature_weight,
        purification_weight=args.dino_purification_weight,
        device=device,
    )
    effective_face_id_backend = args.face_id_backend if face_id_model is not None else "none"
    if (
        args.face_id_backend != "none"
        and (args.real_face_id_weight > 0 or args.real_face_id_drift_weight > 0)
        and face_id_model is None
    ):
        print(
            "warning: real face-ID weights are enabled, but FaceNet did not load; real face-ID loss will be inactive.",
            file=sys.stderr,
        )
    if (args.clip_feature_weight > 0 or args.clip_purification_weight > 0) and clip_surrogate is None:
        print(
            "warning: CLIP surrogate weights are enabled, but the CLIP vision surrogate did not load; CLIP surrogate loss will be inactive.",
            file=sys.stderr,
        )
    if (args.dino_feature_weight > 0 or args.dino_purification_weight > 0) and dino_surrogate is None:
        print(
            "warning: DINO surrogate weights are enabled, but the DINOv2 surrogate did not load; DINO surrogate loss will be inactive.",
            file=sys.stderr,
        )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_loader:
            ...
            epoch_metrics["clip_confusion"] += float(clip_confusion_loss.item())
            epoch_metrics["clip_purification"] += float(clip_purification_loss.item())
            epoch_metrics["dino_confusion"] += float(dino_confusion_loss.item())
            epoch_metrics["dino_purification"] += float(dino_purification_loss.item())
        ...
        averaged_val = _evaluate(
            ...
            clip_surrogate=clip_surrogate,
            dino_surrogate=dino_surrogate,
        ...
        log_row = {
            ...
            "config": {
                ...
                "clipSurrogate": clip_surrogate.model_name if clip_surrogate is not None else None,
                "dinoSurrogate": dino_surrogate.model_name if dino_surrogate is not None else None,
            },
        }
