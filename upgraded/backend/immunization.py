        ...
        if config.denoiser_strength > 0 and reference_data["denoiser_timesteps"]:
            ...
            for timestep, noise, original_noise_predictions in zip(
                ...
                for prompt_embeds, original_noise_prediction in zip(
                    ...
                    denoiser_total = denoiser_total + (guided_noise - original_noise_prediction).norm()
                    denoiser_count += 1
            if denoiser_count > 0:
                denoiser_loss = denoiser_total / float(denoiser_count)
...
class StableDiffusionImmunizationProfile(LayeredDefenseImmunizationProfile):
    ...
    def augment_reference_data(
        ...
        if config.reference_confusion_strength > 0 and protected_mask is not None:
